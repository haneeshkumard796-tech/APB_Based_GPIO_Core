...
module GPIO_registers(sys_clk,sys_rst,gpio_we,gpio_addr,gpio_dat_i,gpio_dat_o,gpio_inta_o,aux_in,out_pad_o,oen_padoe_o,in_pad_i,gpio_eclk);
	...
	generate
		...
		always@(posedge sys_clk or negedge sys_rst) begin
			...
			else if(RGPIO_ECLK[i]) begin : update_at_eclk 
		...
		always@(posedge sys_clk or negedge sys_rst) begin
			...
			else if(sys_rst== 1'b1 && gpio_we == 1'b1 && gpio_addr == 32'h1c)
				RGPIO_INTS[i] <= gpio_dat_i[i];
			
			else if(RGPIO_CTRL[0] == 1'b1 && gpio_we == 1'b0 && sys_rst) begin
	...
	always@(posedge sys_clk or negedge sys_rst) begin
		...
		else if(sys_rst == 1'b1 && gpio_we == 1'b1)	
			case(gpio_addr)
