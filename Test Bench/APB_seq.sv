...
task aux_output_seq::body();
	...
	start_item(req); // RGPIO_AUX
	req.randomize() with {PRESETn==1'b1; PWRITE==1'b1; PADDR==32'h14; PWDATA==32'hffff;};
	finish_item(req);
...
task poll_input_seq::body();
	...
	// 4. Read RGPIO_IN: Poll input values
	start_item(req);
	req.randomize() with {PRESETn==1'b1; PWRITE==1'b0; PADDR==32'h0;};
	finish_item(req);

	// 5. Read RGPIO_INTS: Poll interrupt status
	start_item(req);
	req.randomize() with {PRESETn==1'b1; PWRITE==1'b0; PADDR==32'h1c;};
	finish_item(req);
endtask
...
task input_interrupt_seq::body();
	...
	// 6. Configure RGPIO_OE: Set completely to 1s (32'hffff_ffff)
	start_item(req);
	req.randomize() with {PRESETn==1'b1; PWRITE==1'b1; PADDR==32'h8; PWDATA==32'hffff_ffff;};
	finish_item(req);

	// 7. Read RGPIO_IN: Read input values
	start_item(req);
	req.randomize() with {PRESETn==1'b1; PWRITE==1'b0; PADDR==32'h0;};
	finish_item(req);

	// 8. Read RGPIO_INTS: Read interrupt status
	start_item(req);
	req.randomize() with {PRESETn==1'b1; PWRITE==1'b0; PADDR==32'h1c;};
	finish_item(req);
endtask
...
task poll_input_eclk_seq::body();
	...
	// 6. Read RGPIO_IN: Poll inputs sampled via ECLK
	start_item(req);
	req.randomize() with {PRESETn==1'b1; PWRITE==1'b0; PADDR==32'h0;};
	finish_item(req);

	// 7. Read RGPIO_INTS: Read interrupt status (should remain 0)
	start_item(req);
	req.randomize() with {PRESETn==1'b1; PWRITE==1'b0; PADDR==32'h1c;};
	finish_item(req);
endtask
...
task input_eclk_interrupt_seq::body();
	...
	// 7. Read RGPIO_IN: Poll input values
	start_item(req);
	req.randomize() with {PRESETn==1'b1; PWRITE==1'b0; PADDR==32'h0;};
	finish_item(req);

	// 8. Read RGPIO_INTS: Check interrupt status register
	start_item(req);
	req.randomize() with {PRESETn==1'b1; PWRITE==1'b0; PADDR==32'h1c;};
	finish_item(req);
endtask
